  ...
  <div>
    ...
    <div class="acForm_comp" v-for="item in competition" :key="item.activity_id">
      ...
      <div style="width: 9%;height: 100%;float:left">
        <el-button class="el-button_comp" type="primary" @click="joinCompetition(item.activityId)">Join</el-button>
        <br>
        <el-button class="el-button_comp" type="info" @click="exitCompetition(item.activityId)">Exit</el-button>
        <br>
  ...
  methods: {
    ...
    joinCompetition(activityId) {
      ...
      this.$axios.get('/activity-records/add/' + userId + "/" + activityId).then((resp) => {
        ...
        } else if (resp.data.code === 602) {
          this.$message({
            message: 'The number of people is full',
            type: 'warning',
            duration: 2000
          });
        } else {
